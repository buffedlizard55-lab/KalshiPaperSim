...
export const VERIFIED_FACTS = Object.freeze([
  ...
  },
  {
    ...
    group: 'Capture',
    status: 'DERIVED',
    fact: 'The desk module now publishes its own capture bound — coverage.captureEviction — and decomposes it exactly, so a desk-season round that stops being reproducible says so instead of silently disappearing',
    value: 'From src/desk-data.js after the 2026-09-22 regeneration: storeLadderCaptures 12571 (storeNonEmptyLadders 7776 + storeEmptyLadders 4795) across 18 twenty-minute batches; laddersBeforeCaptureSelection 7776 → evictedByCaptureCap 5762 → laddersAfterCaptureSelection 2014; evictedByMarketCap 1036 across marketsDroppedByCap 102; moduleLadderCaptures 978 in moduleCaptureBatches 8 (moduleEarliestCapture 2026-09-18T18:44:58.204Z, moduleNewestCapture 2026-09-21T20:42:50.633Z). 7776 − 5762 = 2014 and 2014 − 1036 = 978, so the two bounds fully account for the loss. storeBatchesRepresentedInModule 8, storeBatchesNotRepresented 10 (each window listed); seasonRoundStamps 18 with seasonRoundStampsEvicted 14 (each stamp listed, from 2026-09-17T23:26:44.528Z to 2026-09-20T03:53:23.470Z); captureInstantsEvictedFromModule 11592, of which nonEmptyCaptureInstantsEvictedFromModule 6798. 7 of the 9 round stamps in the season committed at 4e85c80 are among the evicted. The block states the rule (newest 12 ladders per market plus one per horizon mark, EVICTED FROM THIS MODULE, never from data/history), the effect, and the reproduction (`node scripts/generate-desk-module.mjs --max-captures 40 --out src/desk-data.js`). Test 139 recomputes both sides independently — from src/desk-data.js and by walking data/history/** — and asserts the decomposition closes.',
    ...
    evidenceLabel: 'src/desk-data.js coverage.captureEviction + scripts/generate-desk-module.mjs selectCaptures() + test 139',
    capturedAt: '2026-09-22',
    usedIn: 'irregularity #64; the desk-season reproducibility note; ROADMAP Next #10; test 139'
  },
  {
    id: 'V126',
    group: 'Trade ledger',
...
,
  ...
  },
  {
    id: 64,
    severity: 'med',
    title: 'The desk module\'s per-market capture cap silently shrank the measured desk-season from 9 rounds to 5 — nothing in the season said why',
    assumed: 'That regenerating src/desk-data.js on newer data could only ADD desk-season rounds.',
    truth: 'selectCaptures() in scripts/generate-desk-module.mjs keeps the newest 12 ladders per market plus one per horizon mark, and the 80-market cap then drops whole contracts, so the module is a sliding WINDOW on data/history/**. The committed season at 4e85c80 (data/reports/desk-season.json, generatedAt 2026-09-21T07:48:47.754Z) measured 9 rounds, 17 fills and 6448 walked events; after the 2026-09-22 regeneration the same season re-reports 5 rounds, 12 fills and 4329 events. Seven of those nine round stamps are gone from the module (2026-09-18T07:04:17.199Z, 11:18:12.556Z, 18:40:54.527Z; 2026-09-19T11:08:18.437Z, 21:26:18.531Z, 21:57:18.998Z; 2026-09-20T00:06:43.226Z) because the module\'s earliest capture is now 2026-09-18T18:44:58.204Z — a round whose batch the module no longer carries cannot be walked again. The full decomposition the module now publishes: the store holds 12571 ladders (7776 non-empty, 4795 empty) in 18 twenty-minute batches; the capture cap keeps 2014 of the 7776 (5762 evicted); the market cap drops 102 contracts carrying 1036 more; the module ends with 978 ladders in 8 batches, leaving 10 store batches unrepresented, 14 of 18 round stamps unreproducible and 11592 capture instants (6798 of them non-empty) absent. Nothing was deleted from data/history/**.',
    evidence: [
      { label: 'the eviction audit the generator now publishes — src/desk-data.js coverage.captureEviction', url: 'https://github.com/buffedlizard55-lab/KalshiPaperSim/blob/main/src/desk-data.js', text: 'storeLadderCaptures 12571, storeNonEmptyLadders 7776, storeEmptyLadders 4795, storeCaptureBatches 18, laddersBeforeCaptureSelection 7776, evictedByCaptureCap 5762, laddersAfterCaptureSelection 2014, evictedByMarketCap 1036, marketsDroppedByCap 102, moduleLadderCaptures 978, moduleCaptureBatches 8, moduleEarliestCapture 2026-09-18T18:44:58.204Z, moduleNewestCapture 2026-09-21T20:42:50.633Z, storeBatchesRepresentedInModule 8, storeBatchesNotRepresented 10, seasonRoundStamps 18, seasonRoundStampsEvicted 14, captureInstantsEvictedFromModule 11592, nonEmptyCaptureInstantsEvictedFromModule 6798.' },
      { label: 'the season it shrank — data/reports/desk-season.json before (9 rounds) and after (5 rounds) the regeneration', url: 'https://github.com/buffedlizard55-lab/KalshiPaperSim/blob/main/data/reports/desk-season.json', text: 'At commit 4e85c80 the committed report reads generatedAt 2026-09-21T07:48:47.754Z, summary.totals.rounds 9, fills 17, eventsWalked 6448, with round stamps 2026-09-18T07:04:17.199Z … 2026-09-20T11:34:47.759Z. Re-run against the 2026-09-22 module it reports rounds 5, fills 12, eventsWalked 4329 — and 7 of the 9 earlier stamps are no longer present among the module\'s capture instants.' },
      { label: 'the cap itself — scripts/generate-desk-module.mjs selectCaptures()', url: 'https://github.com/buffedlizard55-lab/KalshiPaperSim/blob/main/scripts/generate-desk-module.mjs', text: 'MAX_CAPTURES = Number(arg("max-captures", 12)); selectCaptures() returns the newest MAX_CAPTURES ladders plus the newest ladder at or before each horizon mark. Before this change the only trace of the drop was a console line in a workflow run log.' }
    ],
    action: 'coverage.captureEviction now names the rule, the effect and the reproduction (`node scripts/generate-desk-module.mjs --max-captures 40 --out src/desk-data.js`), and publishes both bounds separately (evictedByCaptureCap, evictedByMarketCap) with the batch windows that are no longer represented and the exact round stamps that were evicted. Test 139 recomputes both sides independently — module ladders and instants from src/desk-data.js, store ladders and non-empty counts by walking data/history/** — and asserts the decomposition closes (7776 − 5762 = 2014; 2014 − 1036 = 978), so the published numbers cannot drift from the tree.',
    userAction: 'If the season must keep its history, either regenerate with a larger cap (`node scripts/generate-desk-module.mjs --max-captures 40 --out src/desk-data.js`, then `node scripts/run-desk-season.mjs`) or give the season an append-only round memory. Read coverage.captureEviction before comparing two season runs — the ladders are all still in data/history/**.'
  },
  {
    id: 65,
    severity: 'low',
    title: 'ROADMAP #34 said "data/history holds zero KXMLBGAME/KXNFLGAME stores" — it does not; and 2 OPEN NHL contracts with 20 usable ladders never reach the desk',
    ...
    userAction: 'Read data/reports/game-window-captures.json — totals, series and deskReserves — and note that the 1031 TRADEABLE_EVENT_UNVERIFIED ladders are tradeable-window ladders whose event window could not be joined, not in-play ladders. ROADMAP Next #12 tracks giving the module a per-series quota so an open game series cannot be crowded out.'
  },
]);
