...
import { runCompetition, getVerifiedCandleMap } from '../src/strategy-runner.js';
...
${stats.total} verified facts (${stats.DOCUMENTED} documented, ${stats.CAPTURED} captured from production, ${stats.NEGATIVE} negative/404 evidence, ${stats.DERIVED} derived by arithmetic, ${stats.OBSERVATION} unexplained observations) · ${IRREGULARITIES.length} flagged irregularities · ${STRATEGIES.length} strategies · ${markets.length} real markets · ${totalBars} captured daily candlesticks across ${Object.keys(candleMap).length} markets (${fullWindowSeries} with a full ${EXTENDED_CAPTURE_META.barCount}-bar window) · 58 automated tests
<!-- AUTO:COUNTS-END -->`;
